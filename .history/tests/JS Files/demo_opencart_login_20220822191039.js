exports.demo_opencart_login = class demo_opencart_login{
    ...
    async initialize(){
        ...
        const item_name = "Login";

        for (let i = 0; i < list_group; i++) {
            const L_name = await list_group.nth(i).textContent();
            // console.log(P_name);
            if (P_name.includes(item_name)) {
                await list_group.nth(i).click();
